...
//    };
    ...
    pub.forgotUsername = function (data, request, callback) {
        ...
        getUsername(data.email, function (error, result) {
            ...
            mail.sendMailFromTemplate(message, path.join('username', file + '.html'), path.join('username', file + '.txt'), replaceValues, function (error, result) {
                callback(error, result);
            });
        });
    };
    ...
    pub.register = function (data, request, callback) {
        ...
        crypto.randomBytes(48, function (error, buffer) {
            ...
            createUser(data, function (error, result) {
                ...
                mail.sendMailFromTemplate(message, path.join('register', file + '.html'), path.join('register', file + '.txt'), replaceValues, function (error, result) {
                    callback(error, result);
                });
            });
        });
