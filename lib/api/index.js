...
module.exports = function (app) {
    ...
    function getControllerPath (fullPathToControllerFile, separator) {
        var modulePath = '';
        ...
        if (splittedFile.indexOf('modules') === -1) {
            return null;
        }
    ...
    pub.parseError = function (error, data) {
        if (error) {
            ...
            if (error instanceof app.ClientError) {
            ...
            if (lxHelpers.getType(error) === 'number') {
                var msg = STATUS_CODES[error] || 'Server error';
                app.logging.syslog.warn('Server error: %s, data: %j', msg, data);
...
//            }
    ...
    pub.initAppControllers = function () {
        var grunt = require('grunt'),
            ...
            libControllers = require(app.config.path.libControllers),
            controllers = grunt.file.expand(pattern),
            result = {};

        lxHelpers.forEach(libControllers, function (controller, controllerName) {
            result['lib/' + controllerName] = controller(app);
        });
    ...
    pub.registerSocketEvents = function (socket, acl, controllers, session) {
        ...
        lxHelpers.forEach(acl, function (controllersNames, modulePath) {
            var pathToModule = path.join(app.config.path.modules, modulePath);

            // iterate over each controller in the acl module
            lxHelpers.forEach(controllersNames, function (actions, controllerName) {
                // get controller

                var controller;
                var controllerPath = getControllerPath(pathToModule + path.sep + controllerName);
                    ...
                    lxHelpers.forEach(actions, function (actionName) {
                        ...
                        if (controller[actionName]) {
                            ...
                            socket.on(eventName, function (data, callback) {
                                ...
                                controller[actionName](data, session, function (error, result) {
