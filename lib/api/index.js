...
module.exports = function (app) {
    ...
    function getControllerPath (fullPathToControllerFile) {
        var modulePath = '';
        var splittedFile = fullPathToControllerFile.split('/');
...
//        splittedFile.pop();
    ...
    pub.initAppControllers = function () {
        ...
        lxHelpers.forEach(controllers, function (controllerFile) {
            if (fs.existsSync(controllerFile)) {
                // get controller path, e.g. example/blog/blog
                var controllerPath = getControllerPath(controllerFile);

                // load init controller and store in result
    ...
    pub.registerSocketEvents = function (socket, acl, controllers) {
        ...
        lxHelpers.forEach(acl, function (modulePath, controllers1) {
            var pathToModule = path.join(app.config.path.modules, modulePath);

            // iterate over each controller in the acl module
            lxHelpers.forEach(controllers1, function (controllerName, actions) {
                // get controller

                var controller = controllers[getControllerPath(pathToModule + '/' + controllerName)];
