...
module.exports = function (config, logging) {
    ...
    pub.register = function (socket, acl) {
        ...
        lxHelpers.forEach(acl, function (modulePath, controllers) {
            var pathToModule = path.join(config.path.modules, modulePath);
            ...
            if (fs.existsSync(pathToModule)) {
                ...
                lxHelpers.forEach(controllers, function (controllerName, actions) {
                    ...
                    if (module[controllerName]) {
                        // init controller
                        var controller = module[controllerName]({config: config, logging: logging});
                        ...
                        lxHelpers.forEach(actions, function (actionName) {
                            ...
                            if (controller[actionName]) {
                                ...
                                logging.syslog.debug('socket: registered event %s', eventName);
                            }
                        });
                    }
                });
