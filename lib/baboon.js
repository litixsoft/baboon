...
module.exports = function (rootPath) {
    var express = require('express'),
        ...
        api = require(config.path.api)({config: config, logging: logging, rights: rights, server: {app: app}}),
...
//                syslog.error('Server error: %s, data: %j' + error.message, data);
//                return 'Server error';
//            }
//
//            if (error.validation) {
...
//
//            // log dev errors
//            syslog.debug('Server error: %s, data: %j' + error.message, data);
//            return 'Server error';
//        }
//
//        return null;
//    }
    ...
    var controllers = api.initAppControllers({config: config, logging: logging, rights: rights});
    ...
    var expressConfig = function () {
        ...
        // Catch all rest api requests and forward to controller
        app.use('/api', function (req, res) {
            ...
            if (rights.userHasAccessTo(req.session.user, route)) {
                ...
                if (controllers[parsedRoute.controllerFullPath]) {
                    controllers[parsedRoute.controllerFullPath][parsedRoute.action](data, function (error, result) {
                        res.send({error: api.parseError(error, data), data: result});
                    });
                } else {
                    res.send(403, 'Wrong url');
                }
    ...
    app.get('/lxNav/getNavData', function (req, res) {
        middleware.session.checkSession(req, res, function () {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        rights.getAclObj((session.user || {}).acl, function (error, result) {
            if (error) {
                syslog.error('rights: Error loading acl for user %s (%s)', (session.user || {}).name, (session.user || {})._id);
                return;
            }

            api.registerSocketEvents(socket, result, controllers);
        });
...
//            }
//
//            socketApi.register(socket, result);
//        });
    });
