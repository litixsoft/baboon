...
module.exports = function (rootPath) {
    ...
    var socketApi = api.socket({config: config, logging: logging, rights: rights});
    ...
    io.sockets.on('connection', function (socket) {
        ...
        socketApi.register(socket, rights.getAclObj((session.user || {}).acl, config));
    });
    ...
    // create default system users when in dev mode
    if (process.env.NODE_ENV === 'development') {
        rights.ensureThatDefaultSystemUsersExists();
    }
