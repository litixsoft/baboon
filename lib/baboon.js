...
module.exports = function (rootPath) {
    ...
    var expressConfig = function () {
        ...
        // logging
        if (config.nodeEnv === 'production') {
            app.use(log4js.connectLogger(logging.express, {level: log4js.levels[config.logging.express]}));
        } else {
            app.use(log4js.connectLogger(logging.express, {level: log4js.levels[config.logging.express], format: ':method :url :status'}));
        }
    ...
    app.get('/api/lxNav/getNavData', function (req, res) {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        rights.getAclObj((session.user || {}).acl, function (error, result) {
            ...
            api.registerSocketEvents(socket, result, controllers, session.user);
        });
    });
