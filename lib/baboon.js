    ...
    function startServer() {
        ...
        server.listen(app.get('port'), function () {
            if (config.config !== 'base') {
                syslog.warn('application server has been started in ' + config.config + ' mode');
            }
            else {
                syslog.info('application server has been started in ' + config.config + ' mode');
            }
        ...
        if (! db) {
            ...
            (function configureApplication() {
                ...
                handlebars.registerPartial('scripts', fs.readFileSync(scriptsHtml, 'utf8')
                    .replace('<!--baboon-socket-->', 'var socket = io.connect(\'' + tmpSocket + '\');'));
