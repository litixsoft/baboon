module.exports = function (basePath, callback) {
    ...
    var express = require('express'),
        ...
        baboonDb = {},
        rotatingLogStream = require('file-stream-rotator'),
        expressLogStream = rotatingLogStream.getStream({filename: './logs/express.log', frequency: 'daily', verbose: false}),
        socketLogStream = rotatingLogStream.getStream({filename: './logs/socket.log', frequency: 'daily', verbose: false});
    ...
    (function createBaboonDb() {
        ...
        if (dbServer.length > 1) {
            // replica set
            syslog.debug('use baboonDb replica set');
            ...
            // single server
            syslog.debug('use baboonDb single server');
            //noinspection JSUnresolvedVariable
    ...
    baboonDb.open(function (err, db) {
        ...
        if (! db) {
            ...
            syslog.info('database baboon connected');
            ...
            (function configureApplication() {
                ...
                function configExpressHandler() {
                    ...
                    //noinspection JSUnresolvedVariable
                    if (config.mode === 'production') {
                        app.use(express.logger({stream: expressLogStream, format: config.logging.express}));
                    }
                    else {
