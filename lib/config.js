...
module.exports = function (rootPath, argv) {
    ...
    function checkDirectory(dir) {
        var pathArr = dir.split(path.sep).reverse();
        var pathStr = '';
    ...
    async.auto({
        checkDir: function (next) {
            rootDir = checkDirectory(path.join(rootPath, settings.filesPath));
            settings.filesPath = rootDir + path.sep;
            next();
        },
        checkDirLogs: ['checkDir', function (next) {
            logDir = checkDirectory(path.join(rootDir, 'logs'));
            next();
        }],
        checkDirDbs: ['checkDir', function (next) {
