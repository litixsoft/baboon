...
module.exports = function (rootPath, notice) {
    ...
    if (config.localAppMode) {
        ...
        config.path.logs = path.join(userHomeDir, formatApplicationName(config.appName), 'logs');
        config.path.settings = path.join(userHomeDir, formatApplicationName(config.appName), 'settings');
    } else {
