    ...
    pub.getLastActivity = function (data, req, callback) {
        ...
        req.getSession(function (error, session) {
            ...
            if (!session.hasOwnProperty('activity')) {
                callback(new Error('Error getLastActivity: activity not found in session'));
                return;
            }

            callback(null, {activity: session.activity});
        });
    };
        ...
        req.getSession(function (error, session) {
            ...
            if (typeof key === 'undefined') {
                ...
                if (!session.data.hasOwnProperty(key)) {
                    callback(new Error(key + ' not found in session'));
                    return;
                }
