...
module.exports = function(baboon) {
    ...
    var AccountError = baboon.AccountError;
    ...
    pub.activate = function(req, res, next) {
        ...
        // req.query.lang
        if (req.params) {
            userRepo.findOne({ token: req.params.token }, { fields: ['_id', 'register_date', 'is_active'] }, function(error, result) {
                if(error) {
                    ...
                    if(dateDiffInDays(new Date(), result.register_date) > 30) {
                        ...
                        userRepo.update({ _id: result._id }, { $set: { is_active: true, token: null } }, { multi: false }, function (error, updateResult) {
