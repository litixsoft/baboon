...
module.exports = function (baboon) {
    ...
    pub.restrictedUser = function (req, res, next) {
        ...
        if (config.rights.enabled && config.rights.masterLoginPage) {
            ...
            if (req.session.user && rights.userIsInRole(req.session.user, 'User') || req.session.user.name === 'sysadmin') {
                next();
            }
            else {
                res.redirect('/account/login');
     ...
     * @param {!Function} next
     */
    pub.restrictedSysAdmin = function (req, res, next) {

        // check session and user when rights is enabled and masterLoginPage is true
        if (config.rights.enabled && config.rights.masterLoginPage) {

            if (req.session.user && req.session.user.name === 'sysadmin') {
                next();
