...
module.exports = function (app) {
    ...
    var authenticate = function (name, password, callback) {
        // Get user for login
        app.rights.getUserForLogin(name, function (error, user) {
            // Check error and result.
            if (error) {
                callback(error, null);
            }
            else if (!user) {
                app.logging.syslog.info('auth: User %s not found!', name);
                ...
                pwd.hash(password, user.salt, function (error, hash) {
                    if ((hash && hash.toString() === user.hash.toString())) {
                        ...
                        app.logging.syslog.info('auth: Incorrect password for user %s!', name);
                        callback(new app.AuthenticationError());
                    }
                });
            }
    ...
    /**
     * getAuthData
     * gets isAuth bool and the name of the user
     *
     * @param req
     * @param res
     */
    pub.getAuthData = function (req, res) {
        res.json({error: null, data: {isAuth: req.session.user.id !== -1, name: req.session.user.displayName || req.session.user.name}});
    };

    /**
     * Restricted route when appAuthModus is enable
     *
     * @param req
     * @param res
     * @param next
     * @returns {*}
     */
    pub.restrictedRoute = function (req, res, next) {
        // restricted routes when appModus is true
        if (app.config.useRightsSystem && app.config.extraLoginPage) {
            return pub.restrictedAuthUserRoute(req, res, next);
        }
    ...
    /**
     * Restricted api when appAuthModus is enable
     *
     * @param req
     * @param res
     * @param next
     * @returns {*}
     */
    pub.restrictedApi = function (req, res, next) {
        // restricted routes when appModus is true
        if (app.config.useRightsSystem && app.config.extraLoginPage) {
            return pub.restrictedAuthUserApi(req, res, next);
        }
    ...
    pub.restrictedAdminRoleRoute = function (req, res, next) {
        // session and user is not guest
        if (req.session.user && app.rights.userIsInRole(req.session.user, 'Admin')) {
            next();
        } else {
            res.redirect('/login');
        }
