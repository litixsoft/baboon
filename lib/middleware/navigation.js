'use strict';

var lxHelpers = require('lx-helpers');
var path = require('path');
var NavigationError = require('../errors').NavigationError;

/**
 * Get navigation for site
 *
 * @returns {*}
 */
module.exports = function (rootPath) {

    // Load navigation from config folder
    var nav = require(path.resolve(rootPath,'config','navigation'));
    var navList;
    var pub = {};

    /**
     * Helper for create navigation object
     *
     * @param value
     * @returns {*}
     */
    var addNavObj = function (value) {

        return {
            title: value.title,
            route: value.route,
            target: value.target,
            level: value.level,
            app: value.app
        };
    };

    /**
     * check and optimize navigation tree
     *
     * @param tmpNav
     * @param current
     * @param type
     */
    var checkNav = function (tmpNav, current, type, level) {

        level += 1;

        lxHelpers.forEach(tmpNav, function (value) {

            // add deep level of tree
            value.level = level;

            // check is current app
            if (value.app !== current) {

                // when not current set target to _self
                value.target = '_self';
            }

            // when list push in flat list
            if (type === 'list') {
                navList.push(addNavObj(value));
            }

            // when children call recursive function
            if (value.children) {
                checkNav(value.children, current, type, level);
            }
        });
    };

    /**
     * Get navigation tree
     *
     * @param req
     * @param res
     * @returns {*}
     */
    pub.getTree = function(req, res) {

        var current = req.body.current || 'main';

        var tmpNav = nav();

        checkNav(tmpNav, current, null, -1);

        return res.json(tmpNav);
    };

    /**
     * Get navigation flat list
     *
     * @param req
     * @param res
     * @returns {*}
     */
    pub.getList = function(req, res) {

        var current = req.body.current || 'main';

        var tmpNav = nav();
        navList = [];

        checkNav(tmpNav, current, 'list', -1);

        return res.json(navList);
    };

    /**
     * Get toplevel of navigation
     *
     * @param req
     * @param res
     * @returns {*}
     */
    pub.getTopList = function(req, res) {

        var current = req.body.current || 'main';

        var tmpNav = nav();
        navList = [];

        lxHelpers.forEach(tmpNav, function (value) {

            value.level = 0;

            if (value.app !== current) {
                value.target = '_self';
            }
            else {
                value.target = '';
            }

            navList.push(addNavObj(value));
        });

        return res.json(navList);
    };

    /**
     * Get all sub links from a top as tree
     *
     * @param req
     * @param res
     * @returns {*}
     */
    pub.getSubTree = function(req, res) {

        if (!req.body.top) {
            throw new NavigationError(400, 'getSubTree', 'Parameter top in body is required!');
        }

        var current = req.body.current || 'main';
        var top = req.body.top;

        var tmpNav = nav();
        var subNav = [];

        lxHelpers.forEach(tmpNav, function (value) {

            if (value.title === top.toUpperCase() && value.children) {
                subNav = value.children;
                checkNav(subNav, current, null, 0);
            }
        });

        return res.json(subNav);
    };

    /**
     * Get all sub links from a top as flat list
     *
     * @param req
     * @param res
     * @returns {*}
     */
    pub.getSubList = function(req, res) {

        if (!req.body.top) {
            throw new NavigationError(400, 'getSubList', 'Parameter top in body is required!');
        }

        var current = req.body.current || 'main';
        var top = req.body.top;

        var tmpNav = nav();
        var subNav = [];

        lxHelpers.forEach(tmpNav, function (value) {

            if (value.title === top.toUpperCase() && value.children) {
                subNav = value.children;
                navList = [];
                checkNav(subNav, current, 'list', 0);
            }
        });

        return res.json(navList);
    };

    return pub;
};

