...
module.exports = function (app) {
    ...
    function addGuestUserToSession (session, callback) {
        ...
        if (app.config.useRightsSystem) {
            app.rights.getUser(-1, function (error, result) {
                if (error) {
                    callback(error);
                    return;
                }
    ...
    pub.checkSession = function (req, res, callback) {
        ...
        if (typeof req.session.user === 'undefined') {
            ...
            // check activity time
            if (app.config.sessionInactiveTime < inactiveDifference || app.config.sessionMaxLife < maxDifference) {
        ...
        pub.checkSession(req, res, function (error) {
            if (error) {
                if (error.code) {
                    ...
                    res.json({error: 'checkSession: unknown error'});
                }
            } else {
                // send ok
    ...
    pub.getLastActivity = function (req, res) {
        ...
        if (!req.session.hasOwnProperty('activity')) {
            res.json({error: 'activity not found in session'});
            return;
        }

        res.json({error: null, data: {activity: req.session.activity}});
    };
...
//                res.json({error: key + ' not found in session'});
                obj = null;
//                return;
            } else {
    ...
    pub.setData = function (req, res) {
        ...
        res.json({error: null, data: req.body.key + ' is saved in session'});
    };
    ...
    pub.deleteData = function (req, res) {
        ...
        if (typeof key === 'undefined') {
            req.session.data = {};
            res.json({error: null, data: 'container session.data deleted'});
            return;
        }
        ...
        if (!req.session.data.hasOwnProperty(key)) {
            ...
            res.json({error: null, data: key + ' is deleted in session'});
        }
    };
