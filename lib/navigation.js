...
module.exports = function (navigationFilePath, config) {
    ...
    var checkNav2 = function (tmpNav, user) {
        ...
        lxHelpers.forEach(tmpNav, function (value) {
            ...
            if (value.children) {
                var childs = checkNav2(value.children,user);
                var val = addNavObj(value);
                val.children = childs;
                if(checkAcl(user.acl,val.route)){
    ...
    pub.getTree = function(data, request, callback) {
        ...
        navList = [];
        var der = checkNav2(tmpNav, request.session.user);
        console.log("############",der);
        checkNav(tmpNav, navList, current, null, -1, request.session.user);

//        navList = checkNav2(tmpNav);
        callback(null,der);
    };
