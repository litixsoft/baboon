        ...
        pub.getPublicFunctionsFromControllers(function (error, rights) {
            ...
            async.each(rights, function (right, next) {
                repo.rights.findOne({name: right.name}, function (error, result) {
                    ...
                    if (!result) {
                        repo.rights.insert({name: right.name, description: right.description, controller: right.controller}, function (error, result) {
                            if (error) {
                                next(error);
                                return;
                            }
                        ...
                        if (result.description !== right.description || result.controller !== right.controller) {
                            repo.rights.update({_id: result._id}, {$set: {description: right.description, controller: right.controller}}, function (error, result) {
                                if (error) {
                                    next(error);
                                    return;
                                }
                        ...
                        if (result) {
                            ...
                            createHash(user, function (error, userWithPasswordHash) {
                                ...
                                if (userWithPasswordHash.name === 'admin') {
                                    userWithPasswordHash.roles = [
                                        results.createSystemRoles.Admin,
                                        results.createSystemRoles.User,
                                        results.createSystemRoles.Guest
                                    ];
                                }
