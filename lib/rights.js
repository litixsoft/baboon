...
//            }
    ...
    pub.getUserForLogin = function (username, callback) {
        ...
        repo.findOne({username: username}, {fields: ['username', 'hash', 'salt']}, function (error, result) {
            if (error) {
...
//                                }
    ...
    pub.ensureThatDefaultSystemUsersExists = function (callback) {
        ...
        async.each(systemUsers, function (user, next) {
            repo.findOne({id: user.id, locked: true}, function (error, result) {
                if (error) {
                    next(error);
                    return;
                }
                ...
                if (result) {
                    ...
                    createHash(user, function (error, userWithPasswordHash) {
                        ...
                        if (userWithPasswordHash) {
                            repo.insert(userWithPasswordHash, function (error, result) {
                                if (error) {
                                    next(error);
                                    return;
                                }
