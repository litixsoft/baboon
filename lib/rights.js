...
module.exports = function (config, logging) {
        ...
        async.auto({
            ...
            getUserRoles: function (next, results) {
                if (lxHelpers.isEmpty(user.roles)) {
                    ...
                    rightsRepo.roles.find({_id: {$in: results.getUser.roles}}, next);
                }
            },
    ...
    pub.addResourceRight = function (resource, options, callback) {
        ...
        if (!lxHelpers.isObject(options)) {
            throw new RightsError('param "options" is not an object');
        }
        ...
        if (!resource || lxHelpers.isEmpty(options)) {
            throw new RightsError('missing param "resource" or param "options" is empty');
        }

        if (!options.group_id && !options.user_id) {
            throw new RightsError('missing param "options.group_id" or missing param "options.user_id"');
        }

        if (!options.role_id && !options.right_id) {
            throw new RightsError('missing param "options.role_id" or missing param "options.right_id"');
        }
