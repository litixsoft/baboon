 ...
 *
 * @param {Object} baboon The config.
 * @param {Object} baboon.rights The rights object with options and db connection string.
 * @param {Object} baboon.path The path object with all app paths.
 * @param {Object} baboon.logging The logging object.
 * @return {Object} An object with methods for using right system.
 */
module.exports = function (baboon) {
    // check parameters
    if (typeof baboon !== 'object') {
        throw new RightsError('Parameter baboon is required and must be a object type!');
    }

    var config = baboon.config;
    var logging = baboon.loggers;
    ...
    /**
     * Returns if the user has access to the controller.
     *
     * @param {!Object} user The user object.
     * @param {Array.<number>=} user.id The internal id of the user.
     * @param {Array.<Object>=} user.acl The acl of the user.
     ...
     * @returns {boolean}
     */
    pub.userHasAccessToController = function (user, controller) {
        // check params
        if (!lxHelpers.isObject(user)) {
            throw new RightsError('param "user" is not an object');
        }
        ...
        if (user.acl) {
            ...
            if (controller.charAt(controller.length - 1) === '*') {
                ...
                // controller can be in more than one right, so go through all
                lxHelpers.objectForEach(user.acl, function (key, right) {
                    hasAccess = hasAccess || (right.hasAccess && right.controller === controller);
                });
            }
     ...
     *
     * @param {Object} user The user object.
     * @param {Object} role The roles object.
     ...
     *
     * @param {Object|string} name The user name.
     * @param {Function} callback The callback. (?Object=, ?Object=).
     */
            ...
            async.each(rights, function (right, next) {
                repo.rights.findOne({name: right.name}, function (error, result) {
                    ...
                    if (!result) {
                        ...
                        if (result.description !== right.description || result.controller !== right.controller) {
                            repo.rights.update({_id: result._id}, {
                                $set: {
                                    description: right.description,
                                    controller: right.controller
                                }
    ...
    pub.ensureThatDefaultSystemUsersExists = function (callback) {
        ...
        async.auto({
            createSystemRoles: function (next) {
                ...
                async.eachSeries(systemRoles, function (role, innerNext) {
            ...
            createSystemUsers: ['createSystemRoles', function (next, results) {
                async.eachSeries(systemUsers, function (user, innerNext) {
