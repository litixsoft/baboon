...
module.exports = function (config, logging) {
    ...
    pub.data = function () {
        ...
        res.getExtendedAcl = function (user, additionalGroups, callback) {
            if (lxHelpers.isEmpty(additionalGroups) || lxHelpers.isEmpty(user)) {
                callback();
                return;
            }
            ...
            if (!lxHelpers.isArray(additionalGroups)) {
                callback(lxHelpers.getTypeError('additionalGroups', additionalGroups, []));
                return;
            }

            async.auto({
                getGroups: function (next) {
                    var groupQuery = (typeof additionalGroups[0] === 'string') ? {name: {$in: additionalGroups}} : {_id: {$in: additionalGroups}};
                    rightsRepo.groups.getAll(groupQuery, next);
                },
                getAllRights: function (next) {
                    rightsRepo.rights.getAll(next);
                },
                ...
                getAcl: ['getGroups', 'getAllRights', 'getUserGroups', function (next, results) {
                    var acl = {};

                    if (results.getGroups && !lxHelpers.isEmpty(results.getGroups)) {
                        acl = pub.getUserAcl(user, results.getAllRights, results.getUserGroups, results.getGroups);
                    }
