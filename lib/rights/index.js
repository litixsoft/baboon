...
//            }
    ...
    pub.getAclObj = function (acl, callback) {
        ...
        if (config && config.useRightsSystem === false) {
            ...
            pub.getPublicFunctionsFromControllers(function (error, result) {
                callback(error, processAcl(result));
            });
        } else {
    ...
    pub.getUserForLogin = function (name, callback) {
        ...
        repo.getOne({name: name}, {fields: ['name', 'hash', 'salt']}, function (error, result) {
            if (error) {
    ...
    pub.getPublicFunctionsFromControllers = function (callback) {
        ...
        glob(pattern, function (error, controllers) {
            ...
            async.each(controllers, function (controllerFile, next) {
    ...
    pub.refreshRightsIdDb = function (callback) {
        ...
        pub.getPublicFunctionsFromControllers(function (error, acl) {
            if (error) {
                callback(error);
                return;
            }
