...
module.exports = function (config, syslog) {
    ...
    pub.setActivity = function (data, request, callback) {
        ...
        request.getSession(function(error, session) {
            ...
            if(!error && session) {
                pub.checkActivitySession(session, function(error, result) {

                    if (!error && result) {
                        callback(null, true);
                    }
                    else {
                        callback(error || new SessionError('Session too long inactive or session expired, regenerate session.'));
                    }
                });
            }
            else {
                callback(error || new SessionError('Unknown error, request.getSession returned no valid session'));
            }
        });
    };
        ...
        request.getSession(function(error, session) {
            ...
            if (!error && session) {
                ...
                if (!session.hasOwnProperty('activity')) {
                    ...
                    pub.checkActivitySession(session, function() {
                ...
                return callback(error || new SessionError('Unknown error, request.getSession returned no valid session'));
            }
        });
    };
            ...
            if (!error && session) {
                ...
                if (typeof key === 'undefined') {
                    ...
                    if (!session.data.hasOwnProperty(key)) {
                        return callback(new SessionError('Key: ' + key + ' not found in session container'));
                    }
                    else {
    ...
    pub.setData = function (data, request, callback) {
        ...
        // check params
        if (!data.key) {
            return callback(new SessionError('Parameter key is required'));
        }

        if (!data.value) {
            return callback(new SessionError('Parameter value is required'));
        }
        ...
        request.getSession(function(error, session) {
            ...
            if (!error && session) {
                ...
                session.data[data.key] = data.value;
