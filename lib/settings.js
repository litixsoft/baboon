 ...
 * @param {!Object} loggers The loggers object.
 * @param {!Object} loggers.syslog The syslogger.
...
module.exports = function (baboon) {
    ...
    if (fs.existsSync(defaultClientSettingsFile)) {
        try {
            ...
            baboon.loggers.syslog.warn('settings: Cannot parse default application settings file: %s', defaultClientSettingsFile);
        }
    }
    ...
    pub.getUserSettings = function (data, request, callback) {
        ...
        if (baboon.config.rights.enabled) {
            ...
            if (request.session.settings) {
                return callback(null, request.session.settings);
            }

            // return default application settings for guest user
            ...
            repo.findOneById(request.session.user._id, {fields: ['settings']}, function (error, result) {
                ...
                callback(error, result.settings || lxHelpers.clone(defaultClientSettings));
            });
        } else {
    ...
    pub.setUserSetting = function (data, request, callback) {
        ...
        if (!lxHelpers.isObject(data) || !lxHelpers.isString(data.key)) {
            return callback(new SettingsError('Param setting is missing or has no/wrong property key'));
        }
    ...
    /**
     * Resets the settings of the current user to the default application settings.
     *
     * @param {Object} data The data object.
     * @param {!Object} request The request object.
     * @param {!Object} request.session The session object.
